...
void voltage_to_char(uint8_t ADC_data)
{
	...
	uint16_t char_voltage[] = {};
	/**This is used to display chars*/
	voltage_f = ( (ADC_data *MAX_v) / bit_mode8) * HUNDRED;
	temp_voltage = (uint16_t) voltage_f;

	//decenas, centenas, unidades
	char_voltage[0] = (temp_voltage / HUNDRED) ;
	char_voltage[1] = (temp_voltage % HUNDRED) / TEN;
	char_voltage[2] = (temp_voltage % HUNDRED);

 	LCD_nokia_goto_xy(0,1);
 	LCD_nokia_send_char(char_voltage[0] + ASCII_INIT); //30U DONDE INICIAN LOS NUMEROS EN ASCII
	LCD_nokia_goto_xy(6,1);
	LCD_nokia_send_char(DOT);//PUNTO
	LCD_nokia_goto_xy(12,1);
	LCD_nokia_send_char(char_voltage[1] + ASCII_INIT);
	LCD_nokia_goto_xy(18,1);
	LCD_nokia_send_char(char_voltage[2] + ASCII_INIT);
