...
void voltage_to_char(uint16_t ADC_data)
{
	...
	voltage_f = ( (ADC_data *3.3) / 65535) * 100;
	temp_voltage = (uint16_t) voltage_f;

	//decenas, centenas, unidades
	char_voltage[0] = (temp_voltage / 100) ;
	char_voltage[1] = (temp_voltage % 100) / 10;
	char_voltage[2] = (temp_voltage % 10);

 	LCD_nokia_goto_xy(0,1);
 	LCD_nokia_send_char(char_voltage[0] + 0x30); //30U DONDE INICIAN LOS NUMEROS EN ASCII
	LCD_nokia_goto_xy(6,1);
	LCD_nokia_send_char(0x2E);//PUNTO
	LCD_nokia_goto_xy(12,1);
	LCD_nokia_send_char(char_voltage[1] + 0x30);
	LCD_nokia_goto_xy(18,1);
	LCD_nokia_send_char(char_voltage[2] + 0x30);
}
